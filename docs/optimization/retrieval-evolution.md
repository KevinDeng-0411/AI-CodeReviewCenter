...
> 评估基线：60 条 golden 查询用例，15 篇 fixture 文档，真实 bge-m3 embedding（Ollama 本地 Metal GPU，128ms/次），真实 DeepSeek v4-flash。
...
| 检索腿 | R@5 | MRR@10 |
|---|---|---|
| **pg_trgm 词法** | **0.450** | **0.442** |
| 纯向量（bge-m3） | 0.975 | 0.912 |
| **fused 混合** | **0.975** | **0.912** |
...
- **向量腿单独已很强**（R@5=0.975）——词法腿 pg_trgm 严重拖累（0.450），RRF 融合后仍被词法腿噪声拉低
- **混合检索持平**：fused R@5=0.975 与纯向量持平（60 条下 RRF 噪声被 0.450 词法腿部分抵消）
...
|---|---|---|---|
| C3 pg_trgm | 0.450 | 0.975 | 0.912 |
| C4 BM25（default tokenizer） | 0.567 | 0.975 | 0.928 |
| C4 BM25 + jieba 分词 | 0.943 | 0.975 | 0.928 |
...
- BM25 default tokenizer 对中文精确查询有改善（0.000→0.250），但 R@5=0.250 仍远不能满足需求（60 条下词法腿整体 0.567）
- semantic_paraphrase 类别（语义改写）仍为 0.000——这是词法检索的固有局限，需要依赖向量腿
- 混合 fused 的 MRR 从 0.912 提升到 0.928（+0.016），因为 BM25 的噪声比 pg_trgm 低
...
| 类别 | n | R@5 | MRR@10 |
|---|---|---|---|
| **chinese_exact** | 12 | **1.000** | 0.778 |
| **english_natural** | 10 | **1.000** | 0.933 |
| **rare_identifier** | 12 | **1.000** | 1.000 |
| **semantic_paraphrase** | 15 | **0.900** | 0.856 |
| **negative** | 8 | **1.000** | 1.000 |
| **cross_doc** | 3 | **1.000** | 0.750 |
| **总体** | 60 | **0.975** | **0.896** |

> **结论**：jieba 让中文 BM25 从"残废"变"可用"，中文精确查询从 0.25 跳到 1.000，语义改写也从 0.000 跳到 0.857。这是全项目投入产出比最高的优化。60 条下 fused R@5=0.975、MRR=0.896，检索质量在更大样本下依然稳健。
...
扫描 `top_k ∈ {3, 5, 8, 10, 15}`，60 条 golden cases，生产路径 `HybridRetriever.search_by_vector`。

### 数据

| top_k | R@5 | MRR@10 | avg 实际条数 | 估算 prompt token |
|---|---|---|---|---|
| **3** | 0.967 | 0.917 | 3.0 | 900 |
| **5（当前）** | **0.975** | **0.896** | 5.0 | 1,500 |
| 8 | 0.975 | 0.881 | 8.0 | 2,400 |
| 10 | 0.975 | 0.898 | 10.0 | 3,000 |
| 15 | 0.975 | 0.901 | 15.0 | 4,500 |

### 决策：保持 k=5

1. **R@5 在 k=3 已饱和（0.967）**——检索质量不是 top_k 限制的，前 3 条已含正确答案
2. **MRR 无单调提升**——3/5/8 波动（0.917/0.896/0.881），10/15 回升（0.898/0.901），是 60 条样本噪声
3. **token 成本线性增长**——900→4,500 token，k=8+ 是纯浪费
4. **为什么不降到 3**：R@5 在 k=3 略低（0.967 vs 0.975），3 条没有安全边际。真实查询比 golden 更难，MRR 波动说明 3 和 5 可能互换——省 600 token 不值得冒险
...
### 结果（60 条 golden 采样 35）

| 指标 | mean | n |
|---|---|---|
| **Faithfulness** | **0.931** | 35（采样） |
| **Answer Relevancy** | **0.793** | 35（采样） |

### 解读

1. **整体健康**：Faithfulness 0.931——回答基本被检索 context 支撑，编造少
2. **Answer Relevancy 0.793**：整体切题。相比 35 条基线略降（0.812→0.793），因新增 25 条含更难的语义改写/跨文档用例
3. **评估采样说明**：60 条全量 RAGAS 评估成本高，采样 35 条控制成本（SAMPLE_LIMIT=35），结果代表总体趋势
...
|---|---|---|---|---|---|
| C3 基线 | pg_trgm | 0.975 | 0.912 | 0.000 | 0.000 |
| C4 BM25 | BM25 default | 0.975 | 0.928 | 0.250 | 0.000 |
