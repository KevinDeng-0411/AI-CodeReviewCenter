...
- **路由准确率 1.000**（60/60，2026-08-07 扩充后重跑，门禁 ≥0.90 通过）
- **修复说明（2026-08-05）**：旧报告 0.914 系 Router 故障产物——`with_structured_output(method="json_mode")`
  要求 prompt 必须含 "json" 字眼，原 prompt 缺失导致每次 DeepSeek 返回 400 → 降级 retrieve，"0.914" 只是
  "永远 retrieve" 的兜底成绩（32/35）。补 "json" + 输出格式示例后路由真实生效，60/60 全对（2026-08-07 扩充 golden 到 60 条后重跑）。
- **重试统计**：旧 artifact（evaluator 分数差阈值对 RRF 无效时）触发率 0.906 异常高，已修为 match_type 检测；
  修复后重跑触发率 **0.0**（32 条命中查询全部首检满意、不重试，符合预期）；弱检索重试路径由 mock 覆盖
- 门禁：路由 ≥0.90 ✅（1.000，60/60）、重试收敛 ✅（重试率 1.9%）
