...
> **条件型平台参考，个人默认不实施。** 只有 S6 已按新路线完成，且确有执行模型补丁的需求、
> 独立 rootless runner 与权威 harness 时，才可重新评审；旧卡不能直接获得授权。
>
> **状态：Future / Locked（未来候选，当前版本禁止实施）。**
>
> 只有按新路线完成 S6、S7 的 sandbox 触发条件和执行环境均满足，并且用户随后对 S7
> 另行明确授权，才能开工。S6 完成不自动授权本阶段。
>
