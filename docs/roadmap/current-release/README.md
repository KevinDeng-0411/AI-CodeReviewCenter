...
| C5 | 文档解析元素感知 + PDF 文字版 + 扫描版显式拒绝 | **下一阶段，未开始；需用户明确要求实施** | [05-文档解析与分块增强](05-文档解析与分块增强.md) |

依赖关系：

```text
C1 → C2 → C3 → C4（BM25）→ C5（解析分块增强）→ Agent 路线仅进入“可评审”
```

C4 完成表示 BM25 词法召回增强已闭环并通过门禁，下一步只允许实施 C5。C5 完成也不表示必须升级
Agent；它只形成未来路线的评审前置条件。
...
- C1、C2、C3、C4 的 `evidence/Cx/manifest.json` 均通过
  `validate_stage_evidence.py`，报告及产物哈希可复现；
