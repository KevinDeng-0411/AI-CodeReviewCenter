...
| RAG 召回 | 查询改写 + 结构分块 + pgvector cosine + BM25 + RRF | C4：ParadeDB pg_search 真 BM25 替换 pg_trgm，三路门禁全过，fused MRR 0.906->0.934；C5：元素感知序列化让 DOCX/HTML/PDF 标题穿到分块层（之前文件上传退化为定长滑窗），扫描版 PDF 显式拒绝 |
...
> 伪装成成功或破坏核心消息。知识库检索采用 pgvector 语义召回与 ParadeDB BM25 词法召回（已从 pg_trgm 升级，35 条 golden 门禁全过），
...
决策 4：两条召回腿（BM25 + pgvector）用 RRF 融合，没有 reranker
  → BM25（C4 真实现）抓词面（类名/版本号/错误码），pgvector 抓语义（同义改写）。
  → 为什么不加 reranker？评估后 MRR 0.934 已高，+0.01 边际收益未实测，torch 依赖与 C5 约束冲突。
  ⇒ 结论：RRF 融合 ≠ 二阶段重排，诚实标为"混合召回融合"。
...
决策 7：为什么不做意图识别？
  ...
  → 为什么不做？用户 90%+ 是知识问题，闲聊极少。意图分类的误判漏检代价（把知识问题误判成闲聊）
    远大于闲聊多等半秒——和 reranker 暂缓是同一套逻辑。
  ⇒ 结论：优化看场景收益，不是看别人有没有。触发条件是闲聊占比 >30% 或多数据源需路由。

决策 8：为什么做 jieba 分词、不做 reranker？
  → 两者都是改检索质量，但收益量级差一个数量级：
    reranker 门禁 MRR+0.01（torch 依赖 + 0.5s 延迟）；jieba 中文精确 R@5 0.25→1.000
    （纯 Python，零依赖，延迟无劣化）。同样是"评估后做"，选收益大的那个。
  ⇒ 结论：优化的优先级看实测收益，不看"看起来更高级"。jieba 先做，reranker 继续暂缓。
```
...
> BM25+RRF 是为了词面和语义互相补充而不引入 reranker 的 torch 依赖，
> Chat 厚/工具薄是因为状态机复杂度集中在检索和流式链路，
> 不做 Agent 是因为还没有模型自主选工具的真实需求，
> 不做意图识别是因为用户 90% 知识问题、加分类引入漏检风险大于收益，
> 做 jieba 是因为它让中文 BM25 从残废变可用（R@5 +0.34），而 reranker 门禁才 +0.01——
> 每个选择都有一个"做或不做"的理由。」
...
可插拔(pg_trgm 回退 / BM25 默认)。35 条 golden 三路对照门禁全过:
...
- 不做 rerank 够用吗?对个人项目规模够；rerank 是大型系统的优化项，不是必须。
...
### 6.12 Reranker 二阶段重排：评估后暂缓（工程判断的展示）

C4 后 fused MRR@10=0.934、R@5=0.957。R@5 近天花板但 MRR 略低，说明“召回到了但部分
没排第一”--这是**排序**问题。reranker（cross-encoder 二阶段重排）对症排序，我做了完整
评估但**当前不实施**，决策记录见 [ADR-0009](../decisions/adr/0009-reranker-deferred.md)。

核心判断点（面试可深挖）：

- **指标先分清**：rerank 改 MRR（排序），不改 R@5（召回）。我的 R@5=0.957 已近天花板，
  rerank 碰不到这个数。目标必须说成“MRR 提升”，不是“召回率提升”--一说错就露怯。
- **cross-encoder vs bi-encoder**：bge-m3 是 bi-encoder（query/doc 各自编码、事后算 cosine，
  丢交互）；reranker 是 cross-encoder（query+doc 拼一起进模型，相关性分数含对方交互）。
  这是精排比 RRF + 向量更准的本质。
- **方案**：RRF 召回 top-15 -> bge-reranker-v2-m3 精排 -> top-5。进程内 eager 加载，
  CPU rerank 15 条约 0.45-0.75s。门禁 MRR@10 >= 0.944（+0.01）且 R@5 不降。
- **为什么暂缓**：（1）sentence-transformers 拉 torch，与 C5“不引视觉模型”拒 torch 有
  张力（reranker 的 torch 是必需非无谓，但需解释）；（2）0.5s 延迟加在首 token 前，削弱
  typed SSE“首 token 快”叙事；（3）MRR 0.934 已高，+0.01 边际收益未实测；（4）“评估后
  暂缓+明确门禁”比“硬加测不出效果的优化”更体现判断力。

面试话术：

> 「我评估了 reranker。先分清指标--rerank 改 MRR 不改召回，我 R@5 已 0.957，目标是把
> MRR 从 0.934 往上推。方案是 RRF 召回 15 条、bge-reranker cross-encoder 精排到 5 条，
> 门禁 +0.01 MRR。但我没实施：torch 依赖和我 C5 拒视觉模型的约束有张力，0.5s CPU 延迟
> 和我 SSE 首 token 快的卖点冲突，边际收益又没实测确认。我把它记成 ADR-0009 Deferred，
> 留了重启条件--真实查询频繁排错、或出现 non-torch rerank 方案、或实验分支实测达标。
> 评估并暂缓，比硬塞一个测不出效果的东西更负责。」

追问弹药：
- 不做会不会被问“为什么没优化”?不会。我展示了指标分清、原理、方案、门禁、取舍五层，
  暂缓是结论不是无知。
- 什么情况你会做?真实查询频繁“召回到但没排第一”，且 RRF 调参解决不了；或 non-torch
  reranker 出现消除约束张力。
- rerank 一定提升吗?不一定。候选池太小没意义；reranker 挂了要能降级到 RRF 顺序。
  没测过就加是赌博。
...
**实测数据**（35 golden cases，真实 bge-m3，default tokenizer）：
...
> 稀有标识符不降，延迟还略降。这个优化的价值远超 reranker——同样是改 MRR，jieba
> 提升 0.28，reranker 门禁才 0.01。」
...
**为什么不做**--和 reranker 暂缓(6.12)同一套逻辑,看场景收益:
...
**真正该花精力的优先级高于意图识别**:检索结果质量(reranker,已有门禁)、分块精度(C5 已做)、
...
> 引入误判漏检(把知识问题误判成闲聊)的代价远大于闲聊多等半秒。这个决策和我不做 reranker、
> 不做 LangGraph 是同一套逻辑:优化要看场景收益,不是看别人有没有。触发条件是闲聊占比>30%、
> 多数据源需路由、或延迟成投诉点,当前都不满足。」

追问弹药:
- 不做会不会被问"为什么没优化"?不会。和 reranker 一样,我展示了评估过程(收益/代价/触发条件),
  不做是结论不是无知。
...
检索层 top_k=5 原是经验值。我做了敏感性分析验证——扫描 top_k ∈ {3,5,8,10,15}，35 条 golden + 真实 bge-m3：
...
三条证据：
1. **R@5 在 k=3 已饱和**（0.986）——检索质量根本不是 top_k 限制的，向量腿本就强，前 3 条已含答案
2. **MRR 无单调提升**——3/5/8 均 0.938，10 降 15 升（35 条样本噪声），调大无质量回报
3. **token 成本线性增长**——900→4500，k=8+ 是纯浪费

面试话术：

> 「我验证过 top_k=5 是不是合理——做了敏感性分析，扫描 3 到 15 档，35 条 golden 评测。
...
- 灵敏度高吗?35 条样本,只够看粗粒度趋势(饱和点/收益递减点),不够定位个位数最优解。golden 扩充到 100+ 才重跑。
- 和 reranker 的关系?都是"数据驱动优化"——reranker 门禁 MRR+0.01 不达标暂缓,top_k 验证饱和保持 5。同一套判断逻辑。
...
**评估**（真实 DeepSeek + bge-m3）：路由准确率 0.914（32/35，误判全是常识→retrieve 安全方向）。重试后 docs 全命中（改写有效）。

面试话术：

> 「我用 LangGraph 做了检索层的智能路由和自我纠错，这是"贴近 Agent"的形态——模型参与检索决策，但不执行工具。路由区分常识问题和需要检索的问题，避免无谓检索；检索不理想时自动改写查询重试。过程中有个重要发现：RRF 融合的分数差检测是无效的——因为相邻排名分数差恒定约 0.0003，好坏查询分布一样，我用 match_type（both 命中 vs 纯 vector）替代。路由准确率实测 0.914，误判方向安全（宁可多检索不漏）。」

追问弹药：
- 为什么用 LangGraph 而不是手写?状态图把路由/重试/收敛表达成可观测的节点边；而且这是引入 LangGraph 的合理场景（模型决策 + 条件循环），不是为用而用。
...
| 前端测试 | `34 passed` |
| 前端 lint/build | 通过 |
| Alembic | 唯一 `head=current=0006` |
| C4 BM25 门禁 | 35 golden cases 三路对照全通过 |
...
> 明确收益。现在提前引入只会扩大状态空间。另外我做过耦合度盘点（ADR-0014）：整个
> 后端只有 config.py 一个文件 import LangChain，领域逻辑全是 duck-typing——LangChain
> 对我来说已经是可随时替换的薄 adapter，升级或换更重的 abstraction 都没有业务收益。
> 项目定位是 Chat、最多贴近 Agent——即使未来做工具循环，手写 20 行 while 循环就够，
> 不需要 Graph 基础设施。S3 的确定性 Graph 双运行时是为完整 Agent 铺路的平台化设计，
> 个人项目规模下是过度工程。」
...
| Python | 3.12 |
| API | 25 个 |
| 数据表 | 8 张 (Conversation / Message / LongTermMemory / Document / KnowledgeChunk / PromptTemplate / AiOperationRecord / AiReadmeDocument) |
| ADR | 10 篇 (0001-0010) |
...
| 单腿候选 | `top_k * 3` |
| RRF | `1/(k+rank)`，k=60 |
| 后端测试 | 285 passed, 2 deselected（含 C4 BM25 35 golden + C5 解析 42 + C6 事件 38） |
| 前端测试 | 36 passed（含 C6 SSE 解析 + 引用渲染） |
| Alembic head | 0006 |
...
| 已实现 rerank | 只有 RRF 融合，reranker 评估后暂缓（ADR-0009），门禁明确 |
...
| 长期领域决策 | `docs/decisions/adr/0001~0010` |
