...
| RAG 召回 | 查询改写 + 结构分块 + pgvector cosine + BM25 + RRF | C3 基线：pg_trgm R@5=0.543、vector R@5=0.957、fused R@5=0.957 MRR=0.906；C4 计划升 ParadeDB pg_search 真 BM25 替换 pg_trgm（`chinese_compatible` tokenizer），类目短板 pg_trgm 中文精确=0.0、语义改写=0.0 |
...
C4 计划用 ParadeDB `pg_search` 真 BM25（`chinese_compatible` tokenizer）替换 pg_trgm，预期：
- 词法腿 `R@5` 从 0.543 升到 ≥ BM25 only 水平（中文精确类从 0.0 大幅提升）；
- fused 的 `MRR@10` 摆脱 pg_trgm 噪声，回到 0.920 上下。
